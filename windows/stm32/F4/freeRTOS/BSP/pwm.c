...
void fric_wheel_pwm_Configuration(void) // TIM4_CH3(PB8)  && TIM4_CH4(PB9)		普通定时器PWM输出
{
	...
	gpio_AFconfig(GPIOB, GPIO_Pin_9 | GPIO_Pin_8, GPIO_OType_PP, GPIO_PuPd_UP, GPIO_AF_TIM4);

	tim.TIM_Prescaler = 84 - 1;
	tim.TIM_CounterMode = TIM_CounterMode_Up; // 向上计数
	tim.TIM_Period = 2500 - 1;				  // 50hz
	...
	TIM_ARRPreloadConfig(TIM4, ENABLE); // ARR可以重装，这里不是频域的控制所以这个有没有都没有关系		与上面的一个道理

	TIM_CtrlPWMOutputs(TIM4, ENABLE); // 使能PWM输出

	TIM_Cmd(TIM4, ENABLE);
	//		CCRx/ARR 就是占空比		ARR就是通过TIM_Period设置
	TIM4->CCR3 = 1000; // 对应1 下
	TIM4->CCR4 = 1000; // 对应2	上
}
