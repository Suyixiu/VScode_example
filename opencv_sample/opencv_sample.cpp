...
int main()
{
	...
	img_temp_ones.copyTo(img_temp);				//浅拷贝 img_temp指向img_temp_ones所指的[1,1,1,1] 4个1 73行改了
	...
	img_temp = img_temp_ones.clone();			//clone是新开一块内存
	...
	cvMoveWindow("bar_window", 2333, 0);											//移动这个窗口
	createTrackbar("R_val", "bar_window", &R_val, R_MAX, bar_callback, &img_temp);	//在bar_window这个窗口上创建名字为R_val的滑条 滑条关联的值是R_val 最大值为R_MAX 回调函数句柄为bar_callback 传入参数img_temp
	createTrackbar("G_val", "bar_window", &G_val, G_MAX, bar_callback, &img_temp);	//															  默认最小值为0							  传入参数可以是一个打包好的元组
	createTrackbar("B_val", "bar_window", &B_val, B_MAX, bar_callback, &img_temp);
	while (char(waitKey(1)) != 'q');

	/* 鼠标的使用 */
	setMouseCallback("show", mouse_callback, &img_temp);							//在show这个窗口联系鼠标 mouse_callback是回调函数句柄 传入参数img_temp
	while (char(waitKey(1)) != 'q');
	...
	imshow("show", img_temp);
	while (char(waitKey(1)) != 'q');
	img_temp.convertTo(img_temp, CV_8U, 255.0/65535, 0.0);				//转换成3位8通道的图片 第三个参数是尺度,也就是原图乘上这个比例,注意是小数 第四个参数是偏置
	imshow("show", img_temp);
	while (char(waitKey(1)) != 'q');
